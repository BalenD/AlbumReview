...
namespace AlbumReview.Data
{
    public class AlbumReviewContext : DbContext
    {
        public DbSet<Album> Albums { get; set; }
        public DbSet<Artist> Artists { get; set; }
        public DbSet<Review> Reviews { get; set; }
        public AlbumReviewContext(DbContextOptions<AlbumReviewContext> options) : base(options)
        {
        ...
        {
            AddEntityAudiotInformation();
            return base.SaveChangesAsync(acceptAllChangesOnSuccess, cancellationToken);
        }

        private void AddEntityAudiotInformation()
        {
            var changedEntities = ChangeTracker.Entries().Where(x => x.Entity is IAuditInfo && (x.State == EntityState.Modified || x.State == EntityState.Added));
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            foreach (var deleteableTypes in deleteableEntityTypes)
            {
                modelBuilder.Entity(deleteableTypes.ClrType).HasIndex(nameof(IDeleteableEntity.IsDeleted));
            }
